...
- `src/map-layers/*.geojson` — Map layers (generated by `pnpm export-layers`). Kept in `src/` (not `public/`) so Vite content-hashes each file, which cache-busts them on deploy. Resolve their URLs via `layerUrl(name)` in `src/common/mapLayers.ts` — never hardcode `./layers/...` paths. roads.geojson is drawn as centerlines (see project memory)
