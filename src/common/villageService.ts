...
async function buildEntries(): Promise<VillageEntry[]> {
	...
	const polygons = (villagesRaw.features as Feature<Polygon>[]).filter(
		(f) => f.geometry != null,
	);
	...
	return (labelsRaw.features as Feature<Point>[]).map((labelFeature) => {
		...
		for (const vf of polygons) {
			const ring = vf.geometry.coordinates[0];
		...
		return {
			villageNumber,
			labelPoint: [lat, lng] as [number, number],
			polygon,
		};
	});
}
...
export async function getVillageAtPoint(
	lng: number,
	lat: number,
): Promise<SearchResultVillage | null> {
	const entries = await getVillageEntries();
	for (const entry of entries) {
		if (!entry.polygon) continue;
		const ring = (entry.polygon.geometry as Polygon).coordinates[0];
		if (ring.length >= 3 && pointInPolygon(lng, lat, ring)) {
			return {
				type: "village",
